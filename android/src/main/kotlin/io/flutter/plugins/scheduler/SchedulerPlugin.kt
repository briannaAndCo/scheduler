...
public class SchedulerPlugin(): FlutterPlugin, MethodCallHandler {
  ...
  companion object {
    ...
    @JvmStatic
    private fun rescheduleCurrentNotification(context: Context, hour : Int, minute : Int) {
  ...
  override fun onMethodCall(@NonNull call: MethodCall, @NonNull result: Result) {
    ...
    when(call.method) {
      "SchedulerPlugin.scheduleNotification" -> {
        val hour = args!![0] as Int
        val minute = args!![1] as Int
        val title = args!![2] as String
        val text = args!![3] as String
        val iconResource = args!![4] as String
        val channelId = args!![5] as String
        val channelName = args!![6] as String
        val channelImportance = args!![7] as Int

        savePreferences(mContext, hour, minute, title, text,
                channelId, channelName, channelImportance)

        scheduleNotification(mContext, hour, minute, title,
                text, channelId, channelName,
                channelImportance)
      }
      "SchedulerPlugin.cancelNotification" -> {
        cancelNotification(mContext)
      }
      "SchedulerPlugin.rescheduleCurrentNotification" -> {
        val hour = args!![0] as Int
        val minute = args!![1] as Int

        rescheduleCurrentNotification(mContext, hour, minute)
      }
